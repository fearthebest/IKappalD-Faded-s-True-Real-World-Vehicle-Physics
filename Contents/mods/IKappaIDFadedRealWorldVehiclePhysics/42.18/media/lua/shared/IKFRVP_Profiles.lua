...
-- Pass 4 (parking / crawl): script steeringClamp/grip + runtime park-assist targets (IKFRVP_BrakeRuntime).
-- Sport/Luxury/Race/CompactSport use class 70% assist; other profiles have per-id targets in BrakeRuntime.
-- Pass 3 (acceleration): class/profile power mults + runtime setEngineFeature (IKFRVP_BrakeRuntime).
-- Pass 2 (brakes): target stop-time model + runtime setBrakingForce (see IKFRVP_BrakeRuntime).
