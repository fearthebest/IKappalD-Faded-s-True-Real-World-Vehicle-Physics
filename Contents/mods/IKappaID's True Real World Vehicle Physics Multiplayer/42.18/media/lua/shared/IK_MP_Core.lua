...
if type(IKappaID_MP) ~= "table" then
    IKappaID_MP = {}
end
if type(IK_MP) ~= "table" then
    IK_MP = IKappaID_MP
end

local M = IKappaID_MP

M.ModId = "IKappaID's True Real World Vehicle Physics Multiplayer"
M.Version = "2.5.0"
...
function M.isMultiplayerSession()
    return type(isMultiplayer) == "function" and isMultiplayer()
end

function M.isActiveHere()
    return M.isMultiplayerSession()
end

function M.isRemoteClient()
...
-- Remote MP clients apply only payloads from sendServerCommand (no local tier math).
function M.clientExecutesServerMirror()
    return M.isRemoteClient()
end

function M.hasServerJvm()
    return type(isServer) == "function" and isServer()
end

function M.hasClientJvm()
    return type(isClient) == "function" and isClient()
end

-- getOnlinePlayers() can throw on dedicated servers before udpEngine is ready; gate calls (no pcall).
M._onlinePlayersReady = false

function M.markOnlinePlayersReady()
    M._onlinePlayersReady = true
end

function M.safeGetOnlinePlayers()
    if not M._onlinePlayersReady or type(getOnlinePlayers) ~= "function" then
        return nil
    end
    return getOnlinePlayers()
end

function M.canQueryOnlinePlayers()
    local online = M.safeGetOnlinePlayers()
    return online ~= nil and online.size ~= nil and online:size() > 0
end

-- Layer A script tune: server/listen only — remote clients mirror server sim, not local sandbox.
function M.runsScriptTune()
    if not M.isActiveHere() then
        return false
    end
    if M.isRemoteClient() then
        return false
    end
    return true
end

-- Layer B per-vehicle sync (engine/brakes): server JVM + listen host client, not remote MP clients.
function M.runsDriverPhysicsSync()
    if not M.isActiveHere() or not M.isEnabled() then
        return false
    end
    if M.isRemoteClient() then
        return false
    end
    if M.hasServerJvm() and not M.hasClientJvm() then
        return true
    end
    if M.hasClientJvm() then
        return true
    end
    return false
end

function M.runsDriverTick()
    return M.runsDriverPhysicsSync()
end

function M.runsTrunkMutation()
    return M.hasServerJvm()
end

function M.runsTrunkClientHooks()
    return M.hasClientJvm()
end

-- MP: tow impulses must run on the server JVM only (CSR tow is client-oriented and is not yielded to in MP).
function M.runsTowImpulse()
    if not M.isActiveHere() or not M.isEnabled() or not M.isTowAssistEnabled() then
        return false
    end
    if M.isMultiplayerSession() then
        return M.hasServerJvm()
    end
    return M.hasClientJvm() or M.hasServerJvm()
end
...
function M.side()
    ...
    if M.hasServerJvm() and M.hasClientJvm() then
        return "listen"
    end
    if M.hasClientJvm() then
        return "client"
...
function M.forEachSeatedDriver(callback)
    if type(callback) ~= "function" then
        return false
    end
    local online = M.safeGetOnlinePlayers()
    if online and online.size and online:size() > 0 then
            for i = 0, online:size() - 1 do
                local player = online:get(i)
                if player and player.getVehicle then
                    local vehicle = player:getVehicle()
                    if vehicle and vehicle.isDriver and vehicle:isDriver(player) then
...
M._serverSandbox = nil

function M.hasServerSandbox()
    return type(M._serverSandbox) == "table"
end

function M.applyServerSandbox(snapshot)
    if type(snapshot) ~= "table" then
        return false
    end
    M._serverSandbox = snapshot
    return true
end

-- Local preset table (server/listen host). Remote clients use server snapshot when present.
function M.localSandbox()
    if SandboxVars and SandboxVars.IK_MP then
        return SandboxVars.IK_MP
    end
    return nil
end
...
function M.sandbox()
    if M.isRemoteClient() then
        return M._serverSandbox
    end
    return M.localSandbox()
end

-- Remote MP clients never read local IK_MP presets (host server snapshot is the only source).
function M.option(name, fallback)
    if M.isRemoteClient() then
        local root = M._serverSandbox
        if root and root[name] ~= nil then
            return root[name]
        end
        return fallback
    end
    local root = M.localSandbox()
    if root and root[name] ~= nil then
        return root[name]
    end
    return fallback
    ...
    if M.isRemoteClient() then
        M.log(
            "release "
                ...
                .. " ("
                .. M.ReleaseMilestone
                .. "): client uses server sandbox + physics mirror; trunk UI mirror; tow on server"
        )
        return
    end
    if M.hasServerJvm() and not M.hasClientJvm() then
        M.log(
            "release "
                ...
                .. " ("
                .. M.ReleaseMilestone
                .. "): dedicated server owns IKappaID tuning — push sandbox to clients on connect"
        )
        return
    end
    if M.hasServerJvm() and M.hasClientJvm() then
        M.log("release " .. M.Version .. " (" .. M.ReleaseMilestone .. "): listen host — server + local sandbox")
    end
end
