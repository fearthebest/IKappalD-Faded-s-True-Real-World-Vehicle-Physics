...
function D.dumpSandbox()
    ...
    if IK_MP.Compat then
        D.line("CSR_EnableTowAssist", IK_MP.Compat.isCSRTowAssistSandboxOn())
    end
...
function D.probeScriptIfWatched(script, source, applied, skipReason)
    ...
    if IK_MP.Tune and IK_MP.Tune.readBaseline then
        D.dumpBaseline(IK_MP.Tune.readBaseline(script), "vanilla")
    end
    if IK_MP.Tune and IK_MP.Tune.buildPlan then
        D.dumpPlan(IK_MP.Tune.buildPlan(script))
    end
    D.sectionEnd()
end
...
function D.enterVehicle(player, vehicle)
    ...
    local script = vehicle.getScript and vehicle:getScript() or nil
    local scriptName = script and C.getScriptFullName(script) or "?"
    local title = "ENTER VEHICLE " .. scriptName
    if C.isRemoteClient() then
        title = title .. " (MP mirror — local buildPlan is diagnostic only)"
    end
    D.separator(title)
    D.dumpSandbox()
    if C.isRemoteClient() then
        D.line("physicsAuthority", "server-sim-only (await PhysicsMirror from server)")
    end
    if script and IK_MP.Tune then
        D.dumpBaseline(IK_MP.Tune.readBaseline(script), "vanilla")
        D.dumpPlan(IK_MP.Tune.buildPlan(script))
    end
    if IK_MP.Physics and IK_MP.Physics.getTargetFields then
        local fields = IK_MP.Physics.getTargetFields(vehicle)
        if fields then
