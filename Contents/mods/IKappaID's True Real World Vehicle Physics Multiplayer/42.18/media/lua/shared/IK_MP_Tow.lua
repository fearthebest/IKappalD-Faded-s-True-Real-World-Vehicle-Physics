...
function W.registerEvents()
    ...
    if not C.runsTowImpulse() or Compat.shouldSkipTowAssist() then
...
function W.boot()
    ...
    local skip, skipReason = Compat.shouldSkipTowAssist()
    local towAuthority = "off"
    if C.runsTowImpulse() then
        towAuthority = C.side()
    end
    C.log(
        "tow module ready build="
            ...
            .. (C.isMultiplayerSession() and Compat.isCSRTowAssistSandboxOn() and " csr_tow_ignored_in_mp" or "")
    )
    W.registerEvents()
    return true
end
